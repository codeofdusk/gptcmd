...
class Gptcmd(cmd.Cmd):
    ...
    def _check_for_updates(self):
        ...
        try:
            my_cli = parse_version(__version__)
            their_cli_str = ConfigManager._fetch_pypi_version("gptcmd")
            if their_cli_str:
                their_cli = parse_version(their_cli_str)
                if my_cli < their_cli:
                    print(
                        f"Newer Gptcmd version {their_cli} is available. "
            ...
            for (
                ...
                mine,
                theirs,
            ) in self.config.get_updatable_provider_packages():
                print(
                    f"Newer {name} version {theirs} is available (you have "
                    f"{mine}). Upgrade with: pip install -U {name}"
                )
        except Exception:
            return
